...
import (
	...
	"github.com/btceasypay/bitcoinpay/crypto/sha3"
)
...
func TestProgpowChanges(t *testing.T) {
	headerHash,_ := hex.DecodeString("ffeeddccbbaa9988776655443322110000112233445566778899aabbccddeeff")
	...
	expMix, _ := hex.DecodeString("6018c151b0f9895ebe44a4ca6ce2829e5ba6ae1a68a4ccd05a67ac01219655c1")
	expHash, _ := hex.DecodeString("34d8436444aa5c61761ce0bcce0f11401df2eace77f5c14ba7039b86b5800c08")
...
func TestProgpowKeccak256(t *testing.T) {
	...
	exphash,_ := hex.DecodeString(exp)
...
func hashForBlock(blocknum uint64, nonce uint64, headerHash []byte) ([]byte, []byte, error) {
...
func speedyHashForBlock(ctx *periodContext, blocknum uint64, nonce uint64, headerHash []byte) ([]byte, []byte, error) {
	...
	mixhash, final := progpow(headerHash, nonce, ctx.datasetSize, blocknum, ctx.cDag, lookup)
	return mixhash, final, nil
}

func TestProgpowHash(t *testing.T) {
	hash:=[32]byte{}
	mixHash, finalHash, _ := hashForBlock(0, 0, hash[:])
	expHash,_ := hex.DecodeString("b3bad9ca6f7c566cf0377d1f8cce29d6516a96562c122d924626281ec948ef02")
	expMix,_ := hex.DecodeString("f4ac202715ded4136e72887c39e63a4738331c57fd9eb79f6ec421c281aa8743")
...
func TestProgpowHashes(t *testing.T) {
	...
	for i, tt := range tests {
		...
		headerHash, _ := hex.DecodeString(tt.headerHash)
		...
		expectFinalHash,_ := hex.DecodeString(tt.finalHash)
		expectMixHash,_ := hex.DecodeString(tt.mixHash)
