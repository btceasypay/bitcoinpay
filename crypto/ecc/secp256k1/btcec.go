...
func (curve *KoblitzCurve) ScalarMult(Bx, By *big.Int, k []byte) (*big.Int, *big.Int) {
	...
	bx, qy, qz := new(fieldVal), new(fieldVal), new(fieldVal)
	...
	for i := 0; i < m; i++ {
		...
		for j := 7; j >= 0; j-- {
			// Q = 2 * Q
			curve.doubleJacobian(bx, qy, qz, bx, qy, qz)

			if k1BytePos&0x80 == 0x80 {
				curve.addJacobian(bx, qy, qz, p1x, p1y, p1z,
					bx, qy, qz)
			} else if k1ByteNeg&0x80 == 0x80 {
				curve.addJacobian(bx, qy, qz, p1x, p1yNeg, p1z,
					bx, qy, qz)
			}

			if k2BytePos&0x80 == 0x80 {
				curve.addJacobian(bx, qy, qz, p2x, p2y, p2z,
					bx, qy, qz)
			} else if k2ByteNeg&0x80 == 0x80 {
				curve.addJacobian(bx, qy, qz, p2x, p2yNeg, p2z,
					bx, qy, qz)
			}
			k1BytePos <<= 1
			k1ByteNeg <<= 1
	...
	return curve.fieldJacobianToBigAffine(bx, qy, qz)
}
...
func (curve *KoblitzCurve) ScalarBaseMult(k []byte) (*big.Int, *big.Int) {
	...
	for i, byteVal := range newK {
		...
		curve.addJacobian(bx, qy, qz, &p[0], &p[1], &p[2], bx, qy, qz)
	}
	return curve.fieldJacobianToBigAffine(bx, qy, qz)
}
