...
func Base58CheckEncode(version []byte, mode string, hasher string, cksumSize int, input string) {
	...
	if hasher != "" {
		...
		case "bitcoinpay":
			if len(version) != 2 {
				ErrExit(fmt.Errorf("invaid version byte size for bitcoinpay base58 check encode. input = %x (len = %d, required 2)", version, len(version)))
			}
			encoded = base58.BitcoinpayCheckEncode(data, version[:])
		case "btc":
			if len(version) > 1 {
...
func Base58CheckDecode(mode, hasher string, versionSize, cksumSize int, input string, showDetails bool) {
	...
	if hasher != "" {
		...
		case "bitcoinpay":
			v := [2]byte{}
			data, v, err = base58.BitcoinpayCheckDecode(input)
			if err != nil {
				ErrExit(err)
			}
