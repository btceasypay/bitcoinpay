...
func TestBitcoinpayBase58CheckEncode(t *testing.T) {
	...
	*/
	for _, addrtest := range testAddresses {
		encoded := base58.BitcoinpayCheckEncode(data, addrtest.ver[:])
		assert.Equal(t, encoded, addrtest.addr)
	}
}

func TestBitcoinpayHd(t *testing.T) {
	//
...
func TestBitcoinpayHdMixnet(t *testing.T) {
	//
	// 1.) Use bx to verify the result
	//
	// $ echo 0c891ae06b952c7c30d741590068aa6ca63b4c9d39846da1|bx mnemonic-new
	...
	// Generate a Bip32 HD wallet for the mnemonic and a user supplied password
	seed := bip39.NewSeed(mnemonic, "")
	bip32.DefaultBip32Version = bx.BitcoinpayMixnetBip32Version
	masterKey, _ := bip32.NewMasterKey(seed)
	publicKey := masterKey.PublicKey()
