...
bx is a command-line tool that provides a variety of commands for key management and transaction construction, such as random "seed" generation, public/private key encoding etc. bx cab be built and distributed as a single file binary, which works like the swiss army knife of bitcoinpay
...
```
$ ./bx
Usage: bx [--version] [--help] <command> [<args>]
...
addr & tx & sign
    ec-to-addr            convert an EC public key to a paymant address. default is bitcoinpay address
