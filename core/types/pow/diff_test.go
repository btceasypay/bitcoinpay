...
func TestCalcCuckaroomNextDiff(t *testing.T) {
	p := &PowConfig{
		BitcoinpayKeccak256PowLimit:     new(big.Int).Sub(new(big.Int).Lsh(common.Big1, 232), common.Big1),
		BitcoinpayKeccak256PowLimitBits: 0x1e00ffff,
		Percent: []Percent{
			{
...
// scale * 2^ 64 / diff is target
//edge bits 24 scale is 48
func TestCalcBitcoinpayKeccak256NextDiff(t *testing.T) {
	p := &PowConfig{
		BitcoinpayKeccak256PowLimit:     new(big.Int).Sub(new(big.Int).Lsh(common.Big1, 232), common.Big1),
		BitcoinpayKeccak256PowLimitBits: 0x1e00ffff,
		Percent: []Percent{
