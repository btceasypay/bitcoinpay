...
// bitcoinpay network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
...
// Message is an interface that describes a Bitcoinpay message.  A type that
...
// messageHeader defines the header structure for all Bitcoinpay protocol messages.
