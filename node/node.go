...
func (n *Node) RegisterService() error {
	if n.Config.LightNode {
		return n.registerBitcoinpayLight()
	}
	return n.registerBitcoinpayFull()
}
...
// register services as bitcoinpay Full node
func (n *Node) registerBitcoinpayFull() error {
	err := n.register(NewServiceConstructor("bitcoinpay",
		func(ctx *ServiceContext) (Service, error) {
			fullNode, err := newBitcoinpayFullNode(n)
			return fullNode, err
		}))
	return err
}

// register services as the bitcoinpay Light node
func (n *Node) registerBitcoinpayLight() error {
	err := n.register(NewServiceConstructor("bitcoinpay-light",
		func(ctx *ServiceContext) (Service, error) {
			lightNode, err := newBitcoinpayLight(n)
