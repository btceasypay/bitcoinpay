...
import (
	...
	"github.com/btceasypay/bitcoinpay/common/hash/btc"
	"github.com/btceasypay/bitcoinpay/common/hash/dcr"
	"reflect"
)
...
func checksum_bitcoinpay(input []byte) []byte {
...
func BitcoinpayCheckEncode(input []byte, version []byte) string {
	return CheckEncode(input, version[:], 4, checksum_bitcoinpay)
}
...
// BitcoinpayCheckDecode decodes a string that was encoded with 2 bytes version and verifies
// the checksum using blake2b-256 hash.
func BitcoinpayCheckDecode(input string) (result []byte, version [2]byte, err error) {
	r, v, err := CheckDecode(input, 2, 4, checksum_bitcoinpay)
	if err != nil {
