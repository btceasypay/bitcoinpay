...
func TestBase58Check(t *testing.T) {
	for x, test := range checkEncodingStringTests {
		...
		default:
			eRes = base58.BitcoinpayCheckEncode([]byte(test.in), ver[:])
		}
		if eRes != test.out {
		...
		default:
			res, version, err = base58.BitcoinpayCheckDecode(test.out)
	...
	_, _, err := base58.BitcoinpayCheckDecode("Axk2WA6M")
	...
	for len := 0; len < 4; len++ {
		...
		_, _, err = base58.BitcoinpayCheckDecode(testString)
