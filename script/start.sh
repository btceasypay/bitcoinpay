...
#2.) the relative path of bitcoinpay executable link sould be the same path of the start.sh link
...
# $ mkdir -p $WORK
# $ cd $WORK
# $ ln -s /where/is/my/bitcoinpay/executable
# $ ln -s /where/is/my/bitcoinpay/start.sh
# $ ./start.sh

echo ./bitcoinpay $net $debug $rpc $path $index $network $mining "$@"
./bitcoinpay $net $debug $rpc $path $index $network $mining "$@"
